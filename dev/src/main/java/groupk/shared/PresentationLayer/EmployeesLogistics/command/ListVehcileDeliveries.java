...
public class ListVehcileDeliveries implements Command {
    @Override
    public String name() {
        return "list vehicle deliveries";
    }

    @Override
    public String description() {
        return "print a vehicle's list of deliveries";
    }

    @Override
    public boolean isMatching(String line) {
        return line.startsWith("list vehicle deliveries");
    }
    ...
    public void execute(String[] command, CommandRunner runner) {
        if (command.length != 4) {
            ...
            System.out.println("> list vehicle deliveries <registration-plate>");
            return;
        }
